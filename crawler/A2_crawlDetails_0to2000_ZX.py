...
if __name__=="__main__":
    ...
    originalList=pd.read_csv("rawData.csv").loc[1052:2000,:].values.tolist()
    ...
    for originalI in range(len(originalList)):
        ...
        originalDf=pd.DataFrame(originalItemArr,\
                                columns=["title","url","uid","time","artcle","keywords"])

        originalDf.to_csv("originalData.csv",sep="|",index=None,mode="a",header=None)
